  ...
  parse(value: unknown): Mem0Config {
    ...
    return {
      ...
      agentMemory: (() => {
        ...
        const parsed: Record<string, { capture: string; recall: string[] }> = {};
        for (const [key, val] of Object.entries(cfg.agentMemory as Record<string, unknown>)) {
          if (!val || typeof val !== "object" || Array.isArray(val)) continue;
          const entry = val as Record<string, unknown>;
          if (typeof entry.capture !== "string" || !Array.isArray(entry.recall)) {
          ...
          const recall = (entry.recall as unknown[]).filter((r): r is string => typeof r === "string");
          if (recall.length === 0) {
            console.warn(`openclaw-mem0: agentMemory[${key}].recall is empty after filtering — SKIPPING (fail-closed).`);
            continue;
          }
          parsed[key] = { capture: entry.capture, recall };
        }
